...
//
// 2.0rc3update1 by Ralf-Peter Nerlich (aka Hubert H.) <early8bitz.de>
...
// 2.0rc3unified by Ralf-Peter Nerlich (aka Hubert H.) <early8bitz.de>
//  - Variants for original PCB (Kris Sekula) and Dirk Wouters PCB
//  - Code for power down of unused Parts in MCU (experimental)
...
#undef UNFIXED
//#undef DW32U4
// For using with the original PCB from Kris Sekula (Arduino Nano, ATmega328p) leave DW32U4 undefined
// For using with the new PCB from Dirk Wouters (ATmega32u4) please enable the following line
#define DW32U4

#ifndef DW32U4
//
// Begin definitions for original PCB (Kris Sekula)
//
// Software Fix for original boards with SRAM /OE=GND (<= v1.6 without fix)
// For unfixed Bords enable the following line
//#define UNFIXED

// Signals to shift register 74HC595
...
//
// End definitions for original PCB (Kris Sekula)
//
#else
//
// Begin definitions for new PCB (Dirk Wouters)
//
// Signals to shift register 74HC595
#define DATA     (2)  // data pin for shift register handling D0-D7
#define ADLO     (0)  // data pin for shift register handling A0-A7
#define ADHI     (4)  // data pin for shift register handling A8-A15
#define SCLK     (1)  // clock ping for all shift registers
#define DAT_LD   (6)  // latch pin for all shift registers 

// Bits for direct port access to shift register
#define DATAPORT PORTD
#define DATABIT  (1)  //PORTD:1
#define ADLOPORT PORTD
#define ADLOBIT  (2)  //PORTD:2
#define ADHIPORT PORTD
#define ADHIBIT  (4)  //PORTD:4
#define SCLKPORT PORTD
#define SCLKBIT  (3)  //PORTD:3
#define DATLDPORT PORTD
#define DATLDBIT (7)  //PORTD:7

// Signals to SRAM
#define WE       (12) // SRAM Write Enable Pin

// Bits for direct port access to SRAM
#define WEPORT PORTD
#define WEBIT    (6)  //PORTD:6

// Other signals
#define EN_RST   (3)  //  internal / external bus control pin
#define LD_BTN  (13)  // load data from SPI button
#define LED_G   (A0)  // RED LED
#define EN_CPLD  (5)  // CPLD enable signal

// Bits for direct port access to LED_G
#define LEDPIN PINF
#define LEDBIT    (7) // PORTF:7

// Gating signals - ignores selected A11 - A15 address pins besed on EPROM Type
#define EN_A11 (A5)   // A11 data pin gating signal 
#define EN_A12 (A4)   // A12 data pin gating signal 
#define EN_A13 (A3)   // A13 data pin gating signal 
#define EN_A14 (A2)   // A14 data pin gating signal 
#define EN_A15 (A1)   // A15 data pin gating signal 

// SPI EEPROM stuff (using default SPI pins except SS)
#define DATAOUT   MOSI  // 16
#define DATAIN    MISO  // 14
#define SPICLOCK   SCK  // 15
#define SLAVESELECT (11)
//
//end definitions for new PCB (Dirk Wouters)
//
#endif
...
void setup() {
  ...
  power_adc_disable();        // Power down unneeded parts in MCU (https://www.gammon.com.au/power)
  power_twi_disable();
  ...
  // Initialize all pins
#ifdef DW32U4
  // After MCU reset /EN_CPLD is in INPUT mode and therefore floating (but should be pulled to HIGH with an external resistor)
  pinMode( EN_CPLD, INPUT_PULLUP); // activates the internal pullup and prepares the pin for immediately HIGH when it is set to OUTPUT mode
  pinMode( EN_CPLD, OUTPUT );      // now /EN_CPLD is OUTPUT and active HIGH
#endif
#ifdef UNFIXED
  ...
  // After MCU reset EN_RST is in INPUT mode and therefore floating
  //    but should be pulled to HIGH with an external resistor (Kris)
  //    or should be gated in CPLD (Dirk)
  ...
  pinMode( EN_RST, OUTPUT );       // now EN_RST is OUTPUT and active HIGH
#endif
  // After MCU reset /WE is in INPUT mode and therefore floating
  //    but should be pulled to HIGH with an external resistor (Kris)
  //    or should be gated in CPLD (Dirk)
  ...
  pinMode( WE, OUTPUT );           // now /WE is OUTPUT and active HIGH
...
#ifdef DW32U4
  // Now all digital pins controlling the emulation in their initialized state
  // We can release the CPLD chip
  digitalWrite(EN_CPLD, LOW);
#endif
  ...
  if (autoLoad) {  // we use the SPI stored image and start emulation
  ...
  if (HW_ver[0] != 'v') {
    ...
    //
    EEPROM.put(ver_cfg, "v1.7"); //Set this to whatever you have on the PCB, if you did the HW mod as per Errata, call it v1.6f or similar   <--------- README
...
void load_SPIEEPROM (unsigned int eprom) {
  ...
  register word spi_address = 0;  // SPI_EEPROM address
  ...
  if (digitalRead(LD_BTN) == LOW) {     // Yeah, the button is pressed
    ...
    if (digitalRead(LD_BTN) == HIGH) {  // Button released after 500ms = short press
      ...
      if (digitalRead(LD_BTN) == LOW) {  // Button not released after (500+1000)ms = long press
        ...
        digitalWrite( LED_G, LOW );      // turn the LED off to indicate the RESET was initiated
